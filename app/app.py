...
IS_VERCEL = os.environ.get('VERCEL_ENV') is not None or os.environ.get('VERCEL') == '1'

# Only import JMeter classes if NOT on Vercel
if not IS_VERCEL:
...
def start_test():
    ...
    if IS_VERCEL:
        ...
        return jsonify({
            ...
            "session_id": sid,
            "status": "running",
            "note": "Running in demo mode on Vercel"
        })
...
def stop_test():
    ...
    if IS_VERCEL:
        ...
        return jsonify({"message": "demo_stopped", "session_id": sid})
...
def status(sid):
    if IS_VERCEL:
        # Return demo data for Vercel
        session = SESSIONS.get(sid, {})
        elapsed = int(time.time() - session.get("started_at", time.time())) if session.get("started_at") else 10
        
        return jsonify({
            "status": session.get("status", "demo"),
            "elapsed_s": elapsed,
            "running_vusers": 5,
            "hits_sec": 12.3,
            "passed_txn": 45,
...
def transactions(sid):
    if IS_VERCEL:
        return jsonify([
            {"timestamp": int(time.time() * 1000), "label": "Demo Request", "elapsed": 150, "success": True},
            {"timestamp": int(time.time() * 1000), "label": "Demo Request", "elapsed": 200, "success": True},
        ])
...
def download_jtl(sid):
    if IS_VERCEL:
        return jsonify({"error": "Downloads not available in demo mode"}), 400
...
def generate_report(sid):
    if IS_VERCEL:
        return jsonify({"error": "Report generation not available in demo mode"}), 400
